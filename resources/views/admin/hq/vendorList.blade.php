                                ...
                                @if(count($vendors))
                                    @foreach($vendors as $vendor)
                                        <tr>
                                            ...
                                            <td>{{ $vendor->created_at->format('F d, Y h:ia') }}</td>

                                            <td><a href="{{route('admin.profile.create',$vendor->id)}}"><span class="badge badge-pink">Add profile</span></a> </td>

                                            <?php
                                            if( $vendor->is_deleted==0)
                                            {
                                            ?>
                                            <td><span class="label label-success" onclick="change_status('{{$vendor->id}}','{{$vendor->is_deleted}}');" ><i class="fa fa-check-circle-o" style="color:white"> &nbsp;Active</i></span></td>
                                            ...
                                            {
                                            ?>
                                            <td><span class="label label-warning"  onclick="change_status('{{$vendor->id}}','{{$vendor->is_deleted}}');"><i class="fa fa-times-circle-o" style="color:black"> &nbsp;Inactive</i></span></td>
                                            <?php
                                            }
                                            ?>
                                           ...
                                           {{-- <td>
                                                <a href="{{ URL::to('users/'.$vendor->id.'/edit') }}" class="btn btn-warning btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</a>
                                                {!! Form::open(['method' => 'DELETE','onsubmit' => "return confirm('".trans("Are you sure want to delete")."');", 'route' => ['users.destroy', $vendor->id] ]) !!}
